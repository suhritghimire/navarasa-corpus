...
"""
============================================================
NAVARASA CLASSIFICATION — MuRIL Training
============================================================
Trains Google's MuRIL-large-cased model on Sanskrit Rasa
classification using full fine-tuning and 5-fold cross-validation.

Configuration:
  - Model   : google/muril-large-cased (BERT-large, 24 layers)
  - Strategy: Full fine-tuning
  - Loss    : Weighted Cross-Entropy
  - GPU     : Optimised for 16GB GPU (gradient checkpointing, fp16)

Usage:
    python train.py

Dataset  : ../../data/Filtered_Agreed_Rasa_Dataset_Standardized.xlsx
Outputs  : saved_models/MuRIL_fold<n>_best/
"""
...
DATA_PATH = os.environ.get(
    "SANSKRIT_DATA_PATH",
    os.path.join(REPO_ROOT, "data", "processed", "SanskritRasaBank_v1.xlsx")
)
...
class CheckpointManager:
    ...
    def save(self):
        # Save relative paths for portability
...
def main():
    print("=" * 60)
    print("NAVARASA CLASSIFICATION — MuRIL Fine-tuning")
    print("=" * 60)
    ...
    if df['label'].isna().any():
        print("  Warning: Some labels are not in the 9-Rasa mapping.")
        print("   Found:", df['Final_rasa'].unique())
        sys.exit(1)
